...
describe('INTÉGRATION - Edge Cases et Situations Complexes', () => {
    ...
    test('devrait gérer reset pendant une partie active', () => {
        ...
        // Reset avec initialLength=1 explicite
        snake.reset(5, 5, { dx: 0, dy: 1 }, 1);
    ...
    test('devrait gérer plusieurs shrink consécutifs jusqu\'à la mort', () => {
        ...
        snake.shrink(1);
        expect(snake.alive).toBe(true);  // 1 segment - encore vivant

        snake.shrink(1);
        expect(snake.alive).toBe(false); // 0 segment -> mort
    });
